...
const FarmDetailPage: React.FC = () => {
  ...
  const farmName = farmSlug ? farmSlug.split('-').map(word => 
    word.charAt(0).toUpperCase() + word.slice(1)
  ).join(' ') : '';
  
  // Găsim produsele fermei
  const farmProducts = mockProducts.filter(product => 
    product.farmName.toLowerCase().replace(/\s+/g, '-') === farmSlug
  );
  
  if (farmProducts.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <h2 className="text-2xl font-semibold mb-4">Ferma nu a fost găsită</h2>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-6">
        <ol className="flex text-sm text-gray-500">
          ...
          <li className="breadcrumb-item">
            <Link to="/farms" className="hover:text-primary">Ferme</Link>
            <span className="mx-2">/</span>
          </li>

...
// Componenta pentru galeria de imagini a produsului
    ...
    <div className="mb-6">
      ...
      <div className="flex space-x-2 overflow-x-auto pb-2">
        {images.map((img, index) => (
          <div 
            key={index} 
            className={`w-20 h-20 rounded-md overflow-hidden cursor-pointer ${selectedImage === index ? 'ring-2 ring-primary' : ''}`}
...
const FarmLocationMap: React.FC<{ location: { lat: number; lng: number } }> = ({ location }) => {
...
const mockReviews = [
  {
    ...
    comment: 'Produse excepționale! Calitate superioară și gustul este fantastic. Voi comanda din nou cu siguranță.',
    verified: true
  },
  {
    id: 2,
    ...
    comment: 'Foarte mulțumit de calitate. Produsele sunt proaspete și livrarea a fost la timp.',
    verified: true
  },
  {
    id: 3,
    ...
    rating: 5,
    date: '1 lună în urmă',
    comment: 'Cea mai bună alegere pentru produse locale! Fermierul este foarte amabil și profesionist.',
    verified: false
  }
];
    ...
    <div className="py-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-semibold">Recenzii clienți ({product.reviewCount})</h3>
        ...
        <div className="bg-gray-50 p-6 rounded-lg mb-6">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-3xl font-bold mb-2">{product.rating}</div>
              <StarRating rating={product.rating} reviewCount={product.reviewCount} />
            </div>
            <div className="text-right">
              <div className="text-sm text-gray-600 mb-2">Distribuția ratingurilor:</div>
              {[5, 4, 3, 2, 1].map(stars => (
                <div key={stars} className="flex items-center text-sm">
                  ...
                  <div className="w-16 bg-gray-200 rounded-full h-2 mx-2">
                    <div className="bg-yellow-400 h-2 rounded-full" style={{width: `${Math.random() * 80 + 20}%`}}></div>
                  </div>
                </div>
              ))}
            </div>
...
const PickupDetailsSection: React.FC<{ product: Product }> = ({ product }) => {
  return (
    <div className="py-6">
      <h3 className="text-xl font-semibold mb-6">Detalii de ridicare</h3>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Program de ridicare */}
        <div className="bg-green-50 p-6 rounded-lg">
          <h4 className="font-semibold text-lg mb-4 text-green-800">Program de ridicare</h4>
          ...
          <div className="mt-4 p-4 bg-white rounded border-l-4 border-green-500">
            <div className="flex">
              ...
              <div>
                <p className="text-sm font-medium text-green-800">Recomandare</p>
                <p className="text-sm text-green-700">Pentru a evita aglomerația, vă recomandăm să veniți între 10:00-12:00 sau 14:00-16:00.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          <h4 className="font-semibold text-lg mb-4">Informații de contact</h4>
          <div className="space-y-4">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">{product.farmName.toLowerCase().replace(/\s+/g, '')}@ferma.ro</p>
              </div>
            </div>
      ...
      <div className="mt-8 bg-blue-50 p-6 rounded-lg">
        <h4 className="font-semibold text-lg mb-4 text-blue-800">Instrucțiuni pentru ridicare</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h5 className="font-medium mb-2">Înainte de a veni:</h5>
          ...
          <div>
            <h5 className="font-medium mb-2">La ferma:</h5>
            <ul className="text-sm text-gray-700 space-y-1">
...
const ProductDetailPage: React.FC = () => {
  ...
  // Găsim produsul cu ID-ul din URL
  const product = mockProducts.find(p => p.id === Number(id));
  
  // Produse similare - aceeași categorie dar altele decât produsul curent
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <h2 className="text-2xl font-semibold mb-4">Produsul nu a fost găsit</h2>
  ...
  const handleAddToCart = () => alert(`S-au adăugat ${quantity} ${product.name} în coș`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-6">
        <ol className="flex text-sm text-gray-500">
          ...
          <li className="breadcrumb-item text-gray-700 font-medium">{product.name}</li>
        </ol>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <p className="text-gray-600 mb-4">De la <span className="font-medium">{product.farmName}</span></p>
          
          <StarRating rating={product.rating} reviewCount={product.reviewCount} />
          
          <div className="my-6">
            <h2 className="text-2xl font-bold text-primary">{product.price.toFixed(2)} lei</h2>
            <p className="text-sm text-gray-500 mt-1">Preț pe kilogram</p>
          </div>
          ...
          <div className="mt-10 border-t border-gray-200 pt-6">
            <h3 className="font-semibold text-lg mb-4">Despre {product.farmName}</h3>
            <p className="text-gray-700 mb-4">
              Afacere locală care produce {formatCategoryName(product.category).toLowerCase()} de înaltă calitate, 
              cultivate cu grijă și atenție la mediul înconjurător. Toate produsele 
              sunt culese la maturitate optimă și livrate proaspete.
            </p>
            <Link to={`/farm/${product.farmName.toLowerCase().replace(/\s+/g, '-')}`} className="text-primary hover:underline">
          ...
          <div className="py-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-50 p-4 rounded">
                ...
                <span className="float-right font-medium">{formatCategoryName(product.category)}</span>
              </div>
              <div className="bg-gray-50 p-4 rounded">
                <span className="text-gray-500">Fermă:</span>
                <span className="float-right font-medium">{product.farmName}</span>
              </div>
              <div className="bg-gray-50 p-4 rounded">
      ...
      {/* Farm location map */}
      <div className="mt-10">
        <h3 className="font-semibold text-lg mb-4">Locația fermei</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {relatedProducts.map(relatedProduct => (
            ...
            >
              ...
              <div className="p-4">
                <Link to={`/product/${relatedProduct.id}`}>
                  <h3 className="font-medium text-gray-800 hover:text-primary">{relatedProduct.name}</h3>
                </Link>
                <p className="text-sm text-gray-600 mb-2">{relatedProduct.farmName}</p>
                <div className="flex justify-between items-center">
                  <span className="font-bold">{relatedProduct.price.toFixed(2)} lei</span>
